...
export function generateStaticParams() {
  return VERTICAL_KEYS.flatMap((vertical) =>
    [...dictionaries.en.verticals[vertical].workOrder, "packages"].map(
      (slug) => ({ vertical, slug }),
    ),
  );
}
...
export async function generateMetadata({
  ...
  const v = dictionaries.en.verticals[vertical];
...
export default async function Page({
  ...
  const { vertical, slug } = await params;
  if (slug === "packages") {
    return <PackagesPage lang="en" vertical={vertical} />;
  }
  return <WorkGallery lang="en" vertical={vertical} slug={slug} />;
}
